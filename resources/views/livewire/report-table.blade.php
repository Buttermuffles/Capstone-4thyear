        ...
        <x-modal title="Generate Report" name="generate-report-modal" wire:ignore>
            ...
            <form wire:submit.prevent="createReport">
                <div class="grid grid-cols-2">
                    ...
                    <!-- Guest Selection Dropdown (Visible only for Guest History Report) -->
                    @if ($type === 'Guest History Report')
                    <div class="col-span-2 mt-2">
                        <label class="block mb-2 text-sm font-medium text-gray-900 dark:text-white">Select Guest</label>
                    ...
                    <!-- Start Date and End Date fields (Visible for other report types) -->
                    @if ($type !== 'Guest History Report')
                    <div class="col-span-2 mt-2">
                        <x-text-field1 type="date" name="startdate" placeholder="Start Date" model="startdate" label="Start Date" />
                        @error('startdate')
                            <p class="text-red-500 text-xs italic mt-1"><i class="fas fa-exclamation-circle"></i>{{ $message }}</p>
                        @enderror
                    </div>
